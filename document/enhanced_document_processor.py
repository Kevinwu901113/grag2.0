...
from document.advanced_clustering import AdvancedClusteringProcessor
...
class EnhancedDocumentProcessor:
    ...
    def __init__(self, config: dict):
        ...
        # 初始化高级聚类处理器
        self.clustering_processor = AdvancedClusteringProcessor(config)
        ...
        # 处理模式配置
        self.processing_mode = config["document"].get("processing_mode", "advanced")  # "advanced" 或 "traditional"
    ...
    def process_documents(self, input_dir: str, work_dir: str, logger) -> Dict:
        ...
        if self.processing_mode == "advanced":
            # 使用高级聚类处理
            topics = self._process_with_advanced_clustering(filtered_documents, logger)
        else:
            # 使用传统处理方式
    ...
    def _apply_redundancy_filter(self, documents: List[Dict], logger) -> List[Dict]:
        ...
        for doc in documents:
            ...
            # 对于句子级处理，需要检查每个句子
            if self.clustering_processor.sentence_level:
                sentences = split_into_sentences(text)
                filtered_sentences = []
                
                for i, sentence in enumerate(sentences):
                    if len(sentence.strip()) >= self.clustering_processor.min_sentence_length:
                        # 生成嵌入向量进行冗余检测
    ...
    def _process_with_advanced_clustering(self, documents: List[Dict], logger) -> List[Dict]:
        """
        使用高级聚类算法处理文档
        ...
        Returns:
            主题列表
        """
        logger.info("开始高级聚类处理...")
        
        # 使用高级聚类处理器
        topics = self.clustering_processor.process_documents(documents)
        
        logger.info(f"高级聚类完成，生成 {len(topics)} 个主题")
        ...
        if len(topics) > 1:
            try:
                ...
                for i, topic in enumerate(topics):
                    topic_embedding = self.llm_client.embed([topic["text"]])[0]
                    embeddings.append(topic_embedding)
                    labels.append(i)
                
                if len(embeddings) > 1:
                    import numpy as np
                    quality_analysis = self.clustering_processor.analyze_clustering_quality(
                        np.array(embeddings), np.array(labels)
                    )
                    logger.info(f"聚类质量分析: {quality_analysis}")
                    
            except Exception as e:
                logger.warning(f"聚类质量分析失败: {str(e)}")
    ...
    def _save_results(self, topics: List[Dict], work_dir: str, logger) -> Dict:
        ...
        stats = {
            ...
            "clustering_method": getattr(self.clustering_processor, 'clustering_method', 'unknown'),
