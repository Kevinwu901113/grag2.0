...
# 处理文档（高级聚类）
python main.py --mode doc --config config.yaml --use_advanced_clustering
...
- **嵌入模型**: 支持多种嵌入模型
- **文档处理**: 分块大小、重叠等参数
- **高级聚类**: 聚类算法、参数配置等（详见 `config_advanced_clustering.yaml.example`）
- **图构建**: 实体提取、关系识别参数
- **检索配置**: 向量检索、图检索、重排序参数

### 高级聚类功能

新增的高级聚类功能支持句子级分解和多种聚类算法：

**支持的聚类算法**：
- **层次聚类（Hierarchical）**: 基于距离的层次聚类，适合发现层次结构
- **谱聚类（Spectral）**: 基于图论的聚类，适合非凸形状的聚类
- **K-means**: 经典的聚类算法，适合球形聚类

**主要特性**：
- 句子级文本分解，避免段落合并的语义混杂
- 全局聚类优化，比在线阈值法更稳定
- 距离矩阵优化，提升聚类质量
- 聚类质量评估（轮廓系数、Calinski-Harabasz指数）

**使用方法**：
```bash
# 使用高级聚类处理文档
python main.py --mode doc --use_advanced_clustering

# 或者运行示例
python advanced_clustering_example.py
```

详细文档请参考：[高级聚类功能指南](docs/ADVANCED_CLUSTERING.md)
